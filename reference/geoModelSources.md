# Extract latitude and longitude of points identified as sources by [`geoMCMC()`](https://emmadeeks97.github.io/GeoProfile/reference/geoMCMC.md)

This function takes the output of
[`geoMCMC()`](https://emmadeeks97.github.io/GeoProfile/reference/geoMCMC.md)
and, for each 'crime', extracts the group to which it is assigned with
the highest probability. For each group, the model returns the mean
lat/long of all crimes assigned to that group.

## Usage

``` r
geoModelSources(mcmc, data)
```

## Arguments

- mcmc:

  Model output in the format produced by
  [`geoMCMC()`](https://emmadeeks97.github.io/GeoProfile/reference/geoMCMC.md).

- data:

  Crime site data, in the format produced by
  [`geoData()`](https://emmadeeks97.github.io/GeoProfile/reference/gp.data.md).

## Examples

``` r
if (FALSE) { # interactive()
# \donttest{
# simulated data
sim <-rDPM(50, priorMean_longitude = -0.04217491, priorMean_latitude =
51.5235505, alpha=10, sigma=1, tau=3)
d <- geoData(sim$longitude, sim $latitude)
s <- geoDataSource(sim$source_lon, sim$source_lat)
p <- geoParams(data = d, sigma_mean = 1.0, sigma_squared_shape = 2)
m <- geoMCMC(data = d, params = p)
# extract sources identified by the model
ms <- geoModelSources(mcmc = m, data = d)
# plot data showing the sources identified by the model (note: NOT the actual suspect sites)
 geoPlotLeaflet(data = d,
                source = ms,
                params = p,
                mapType = 110,
                surfaceCols = c("red", "orange","yellow","white"),
                crimeCol = "black",
                crimeCex = 2,
                sourceCol = "red",
                sourceCex = 2,
                surface = m$geoProfile,
                gpLegend=TRUE,
                opacity = 0.4)
# }
}
```
